...
- Real workspace changes stop automatic retry and Provider fallback. A same-session, no-tool finalization may still return the already-completed write result only when it leaves the post-task workspace unchanged and its initial Provider process exited naturally; any finalization tool event, Worker-forced write timeout, or new change remains `partial`.
...
| --- | --- | --- |
| Stream faults | malformed, guessed, no-tool, error, missing-finish, length-truncated streams, and same-session finalization recovery | no invalid result accepted; recovery never replays a task. A write can finalize only after a natural initial exit and without finalization tools or post-task workspace changes. |
